...
Given('all categories are cleared', () => {
  ...
  cy.get('body').then(($body) => {
    ...
    const deleteButtons = $body.find('table tbody tr button, table tbody tr a').filter(function() {
      const text = Cypress.$(this).text().toLowerCase();
      return text.includes('delete') || text.includes('trash') || text.includes('remove') || text === '×';
    });
    ...
    cy.log(`Found ${deleteButtons.length} delete actions - attempting to clear categories`);
  });
});
...
Then(/the\s+Search\s+sub\s*category\s+input\s+field\s+should\s+be\s+visible/i, () => {
  CategoriesViewPage.verifySearchInput();
});
...
Then('I should see {string} message', (message) => {
  cy.contains(new RegExp(message, 'i')).should('be.visible');
});
...
Then('I should see table with columns {word}, {word}, {word}, {word}', (col1, col2, col3, col4) => {
  CategoriesViewPage.verifyTableColumns([col1, col2, col3, col4]);
});

Then('I should see table with columns {word}, {word}, {word}', (col1, col2, col3) => {
...
Given('categories with parents exist in the system', () => {
  ...
  CategoriesViewPage.elements.allParentsDropdown().should('have.length.greaterThan', 1);
});
...
Given('a category named {string} exists', (categoryName) => {
  ...
  cy.get('input[name="name"], input[placeholder*="Category name"], #categoryName').type(categoryName);
  ...
  cy.wait(1000);
  cy.visit('/ui/categories');
  cy.get('table', { timeout: 10000 }).contains('td', categoryName).should('be.visible');
});
...
When('I enter an invalid category name {string} in the search field', (name) => {
...
When('I enter a valid category name {string} in the search field', (categoryName) => {
...
When('I select a parent category from the dropdown', () => {
  ...
  CategoriesViewPage.elements.allParentsDropdown().then(($select) => {
    const $opts = Cypress.$($select).find('option').filter((i, el) => {
...
Then('the search result should contain {string}', (searchTerm) => {
  ...
  cy.get('body').then(($body) => {
    if ($body.find('table').length > 0) {
      cy.get('table', { timeout: 10000 }).contains('td', searchTerm, { timeout: 10000 }).should('be.visible');
    } else {
      cy.contains(/no categories found|no data|no results/i, { timeout: 5000 }).should('be.visible');
    }
  });
});
